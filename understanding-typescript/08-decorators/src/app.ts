...
class Product {
  ...
  set price(val: number) {
    if(val > 0) {
      ...
      throw new Error('Invalid price should be positeive')
    }
  }
...
interface ValidatorConfig {
  [property: string]: {
    [validatableProp: string]: (string | string[])[] // ['required', 'positive']
  }
}
...
function validate(obj: any) {
   ...
   for(const prop in objValidatorConfig) {
    for(const validator of objValidatorConfig[prop]) {
      switch(validator) {
        case 'required': 
          isValid = isValid && !!obj[prop]; break;
        case 'positive':
          isValid = isValid && obj[prop] > 0; break;
      }
    }
   }
   return isValid;
